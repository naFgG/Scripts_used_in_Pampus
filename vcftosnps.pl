...
impossible:while (my $line = readline($IN_FILE)) {
    ...
    my $missingcnt_pct = sprintf("%.2f", $missingcnt/($arraylen-9)*100);
    ...
    if (($idlag)&&($id ne $idlag)) { 
    	# save the best SNP in previous gene
    	my @gene_snps = sort keys %gene_snps;
    	my $gene_snps_num = scalar @gene_snps; 
    	my $rand_select = int(rand($gene_snps_num));
    	my $entry = $gene_snps[$rand_select];
	    ...
	    for (my $i = 9; $i < $arraylen; $i ++) {
            ...
            #print "$snps{$tmp2[0]}->{$head[$i]}->{1}-$snps{$tmp2[0]}->{$head[$i]}->{2}\t";
        }
        #print "$tmp2[1]\t$tmp2[5]\n";
...
my $ntax = 2 * scalar(@taxalist);
my $nchar = scalar(@loci);
my $stax = scalar(@taxalist);
